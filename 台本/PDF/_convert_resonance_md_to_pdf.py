"""共鳴トレーニングセリフ集 Markdown → PDF変換スクリプト

テーブル・コードブロック・引用ブロック・見出し階層に対応。

Usage:
    python _convert_resonance_md_to_pdf.py <input.md>
"""
import sys, re
...
header_text = first_line.lstrip("# ").strip() if first_line.startswith("#") else "共鳴トレーニング セリフ集"
...
def draw_code_block(pdf, code_lines):
    line_h = 5
    pad = 4
    code_w = pdf.epw - pad * 2
    pdf.set_font("yumin", style="", size=8.5)

    total_text_h = 0
    for cl in code_lines:
        sw = pdf.get_string_width(cl) if cl.strip() else 0
        n = max(1, int(sw / code_w) + (1 if sw % code_w > 0 else 0))
        total_text_h += line_h * n

    block_h = total_text_h + 6
    needed = block_h + 8
    if pdf.get_y() + needed > pdf.h - pdf.b_margin:
        pdf.add_page()
    ...
    for cl in code_lines:
        pdf.set_x(x + pad)
        pdf.multi_cell(w=code_w, h=line_h, text=cl if cl.strip() else " ",
                        align="L", new_x="LMARGIN", new_y="NEXT")
...
def draw_quote_block(pdf, quote_lines):
    ...
    pad_y = 3
    accent_w = 2.5
    text_x = pdf.l_margin + pad_x
    text_w = pdf.epw - pad_x - 2

    joined = [strip_bold(ql) for ql in quote_lines]
    pdf.set_font("yumin", style="", size=font_size)
    total_text_h = 0
    for tl in joined:
        if not tl.strip():
            total_text_h += line_h * 0.5
            continue
        sw = pdf.get_string_width(tl)
        n_lines = max(1, int(sw / text_w) + (1 if sw % text_w > 0 else 0))
        total_text_h += line_h * n_lines

    block_h = total_text_h + pad_y * 2 + 2
    needed = block_h + 4
    if pdf.get_y() + needed > pdf.h - pdf.b_margin:
        pdf.add_page()
    ...
    pdf.rect(x, y, accent_w, block_h, style="F")
    ...
    for ql in quote_lines:
        ...
        pdf.set_x(text_x)
        pdf.set_font("yumin", style="", size=font_size)
        pdf.multi_cell(w=text_w, h=line_h, text=plain,
                        align="L", new_x="LMARGIN", new_y="NEXT")
